...
int eredmeny(int p1_eredmeny, int p2_eredmeny){
	if((p2_eredmeny>21 && p1_eredmeny>21) || (p1_eredmeny==p2_eredmeny))
		return 3; //döntetlen
	else if(p2_eredmeny > 21 || (21-p1_eredmeny) < (21-p2_eredmeny))
		return 1;
	else if(p1_eredmeny > 21 || (21-p1_eredmeny) > (21-p2_eredmeny))
		return 2;
	else
		return 3;
}
	...
	{	
		...
		for(;;)
		{
			...
			if(!voltPassz_player1 && player1_osszeg < 21){
				...
				legfelsoLapIndex++;
				if(Pakli[lapIndex][1] == 'Q' || Pakli[lapIndex][1] == 'J' || Pakli[lapIndex][1] == 'K')
					player1_osszeg += 10;
				else if(Pakli[lapIndex][1] == 'A')
					if(player1_osszeg < 11)
						player1_osszeg +=11;
					else
						player1_osszeg +=1;
				else{
